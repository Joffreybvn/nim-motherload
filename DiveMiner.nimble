...
task runr, "Runs DiveMiner for current platform":
 exec &"nim c -r {releaseOpts} -o:DiveMiner src/main.nim"

task rund, "Runs debug DiveMiner for current platform":
 exec &"nim c -r {debugOpts} -o:DiveMiner src/main.nim"

task release, "Builds DiveMiner for current platform":
 exec &"nim c {releaseOpts} -o:DiveMiner src/main.nim"

task webd, "Builds debug DiveMiner for web":
 exec &"nim c {debugOpts} -d:emscripten -o:DiveMiner.html src/main.nim"

task webr, "Builds release DiveMiner for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:DiveMiner.html src/main.nim"

task debug, "Builds debug DiveMiner for current platform":
 exec &"nim c {debugOpts} -o:DiveMiner_debug src/main.nim"
